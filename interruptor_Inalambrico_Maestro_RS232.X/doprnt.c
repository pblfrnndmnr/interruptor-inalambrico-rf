...
dostring:
		...
		if(flag & (FLOATFORMAT)) {
			...
			if( fval!=0) {		/* If the number is non-zero, find the exponent. */
				...
				integ = scale(-exp);		// replaced with this
				integ *= fval;
				if(integ < 1.0)
					exp--;
				else if(integ >= 10.0)
		...
		{
#ifdef	__LONG
#if	defined(_PIC18) && defined(LARGE_DATA) && defined(POINTER)
			if(flag & POINTER)
